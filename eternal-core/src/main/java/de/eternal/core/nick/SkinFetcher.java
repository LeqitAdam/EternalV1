 ...
 * lifetime of the process; failures return null and the nick degrades to
 * name-only.
 *
 * <p>Lives in core so BOTH the Spigot backend and the BungeeCord proxy can use
 * it. On a network the proxy is the natural place to fetch (it is online-mode
 * and authenticated), but the HTTP calls work from either side regardless of a
 * backend server's online-mode setting — fetching a public skin is just an
 * outbound HTTPS request, not an auth operation.</p>
 */
public final class SkinFetcher {
    ...
    private @Nullable JsonObject getJson(@NotNull String url) throws Exception {
        ...
        conn.setRequestProperty("User-Agent", "Eternal");

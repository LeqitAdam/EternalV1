  ...
  template: `
    ...
    <div *ngIf="!loading()" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
      <mat-card *ngFor="let a of appeals()" class="p-5">
        ...
        <div *ngIf="a.status === 'PENDING' && auth.isAdmin()" class="flex gap-2">
          <button mat-flat-button color="primary" (click)="approve(a)">
            <mat-icon>check</mat-icon> Genehmigen + Entbannen
          </button>
          <button mat-stroked-button color="warn" (click)="deny(a)">
            <mat-icon>close</mat-icon> Ablehnen
          </button>
        </div>

        <div *ngIf="a.status !== 'PENDING'" class="text-sm text-ink-300">
          {{ a.status === 'APPROVED' ? 'Genehmigt' : 'Abgelehnt' }} von {{ a.reviewerName }}
...
export class AppealsComponent implements OnInit {
  ...
  readonly tab = signal<'PENDING' | 'APPROVED' | 'DENIED'>('PENDING');

  ngOnInit() { this.loadTab(0); }

  loadTab(idx: number) {
    this.tab.set(idx === 0 ? 'PENDING' : idx === 1 ? 'APPROVED' : 'DENIED');
    this.loading.set(true);
    this.api.appeals(this.tab()).subscribe({

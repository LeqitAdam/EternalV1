...
public final class FlyCommand implements CommandExecutor, TabCompleter {
    ...
    public FlyCommand(@NotNull EternalSpigot plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        Player target;
        if (args.length >= 1) {
            if (!Cmd.has(sender, "eternal.base.fly.others")) {
        ...
        if (target.equals(sender)) {
            plugin.messages().send(sender, "fly-self", "state", state);
        } else {
            plugin.messages().send(sender, "fly-other", "player", target.getName(), "state", state);
            plugin.messages().send(target, "fly-self", "state", state);
        }
        return true;
    }
